...
//!
//! The payload is produced/consumed by a [`crate::client::codec::Codec`] (JSON or Protobuf).
    ...
    S: AsyncRead + AsyncWrite,
{
    ...
    pub async fn send_frame(&mut self, frame: ClientFrame) -> Result<(), ClientTransportError> {
        let span = crate::client::span_for_frame("client.framed.send", &frame);
        let _enter = span.enter();

        let bytes = self.codec.encode_frame(&frame)?;
        let len = bytes.len();
        if len > self.max_frame_len || len > u32::MAX as usize {
            return Err(ClientTransportError::FrameTooLarge {
                len,
    ...
    pub async fn recv_frame(&mut self) -> Result<ClientFrame, ClientTransportError> {
        ...
        self.read
            ...
            .await
            .map_err(super::map_read_exact_error)?;
        ...
        let span = crate::client::span_for_frame("client.framed.recv", &frame);
        let _enter = span.enter();
        Ok(frame)
    }
}

impl<C, S> ClientConnection for FramedEndpoint<C, S>
where
    C: Codec + 'static,
