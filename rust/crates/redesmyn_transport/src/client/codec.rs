...
pub trait Codec: Send + Sync {
    ...
    fn encode_frame(&self, frame: &ClientFrame) -> Result<Vec<u8>, CodecError>;
    fn decode_frame(&self, bytes: &[u8]) -> Result<ClientFrame, CodecError>;
}
...
impl Codec for ProtobufCodec {
    ...
    fn decode_frame(&self, bytes: &[u8]) -> Result<ClientFrame, CodecError> {
        let proto = pbv1::ClientFrame::decode(bytes)?;
        ClientFrame::try_from_protobuf(proto).map_err(|error| FrameDecodeError { error }.into())
    }
}
